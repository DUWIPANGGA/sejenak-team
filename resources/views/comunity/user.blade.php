    ...
    <div class="w-full min-h-full h-full flex flex-col md:flex-row gap-6 px-8 py-0 justify-center items-start overflow-auto">
        ...
        <aside class="left-sidebar hidden md:block w-full md:w-1/4 lg:w-1/5 h-full">
        ...
        <main class="main-content w-full h-full overflow-auto md:w-2/4 lg:w-1/2 flex-grow flex flex-col items-center gap-6 p-0 m-0 scrollbar-none">
            ...
            <div class="post-card w-full max-w-2xl bg-primary p-6 rounded-playful-lg border-2 border-dark shadow-border-offset">
                <div class="flex items-center mb-4 relative">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-sm text-black font-lexend">@anonymous • {{ $post->created_at->diffForHumans() }}</p>
                    </div>
                    @else
                    {{-- Skenario 2: Postingan pengguna (dengan atau tanpa avatar) --}}
                    @if($post->user->avatar)
                    <img src="{{ asset($post->user->avatar) }}" alt="{{ $post->user->name }}" class="w-10 h-10 rounded-full border-2 border-dark shadow-border-offset mr-3 object-cover">
                    @else
                    <div class="w-10 h-10 flex items-center justify-center rounded-full border-2 border-dark shadow-border-offset mr-3 font-bold text-white {{ $randomColor }}">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-sm text-gray-500 font-lexend">{{ strtolower($post->user->name) }} • {{ $post->created_at->diffForHumans() }}</p>
                    </div>
                    @endif

                    {{-- New: Three-dot menu with dropdown --}}
                    <div class="absolute top-0 right-0">
                        <button class="text-dark p-2 rounded-full hover:bg-gray-200 transition-colors" onclick="toggleDropdown('dropdown-{{ $post->id }}')">
                        ...
                        <div id="dropdown-{{ $post->id }}" class="hidden absolute right-0 mt-2 w-40 bg-white border-2 border-dark rounded-lg shadow-lg z-10">
                            {{-- Opsi dropdown hanya muncul jika pengguna adalah pemilik postingan --}}
                            @if($post->user_id == auth()->id())
                            <a href="" class="block px-4 py-2 text-sm text-dark hover:bg-gray-100">Edit</a>
                            <form action="{{ route('user.comunity.destroy', $post->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus postingan ini?');">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="block w-full text-left px-4 py-2 text-sm text-dark hover:bg-gray-100">Hapus</button>
                            </form>
                            @endif
                            <a href="" class="block px-4 py-2 text-sm text-red-500 hover:bg-gray-100">Laporkan</a>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="flex items-center gap-4 w-30">
                        <div class="like-container">
                            <button class="flex items-center gap-1 text-dark hover:text-red-500 transition-colors like-button" data-post-id="{{ $post->id }}" data-liked="{{ $post->isLikedByUser() ? 'true' : 'false' }}">
                                ...
                                @else
                                <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-7 h-7 like-icon text-gray-500">
                                ...
                                @endif
                                <span class="font-lexend text-sm like-count like-count-animation">{{ $post->likes->count() }}</span>
                            </button>
                        </div>

                        <button class="flex items-center gap-1 text-dark hover:text-blue-500 transition-colors comment-button" data-post-id="{{ $post->id }}" onclick="openCommentModal({{ $post->id }})">
                            <img src="{{ asset('assets/component/emote/chat_icon.svg') }}" alt="Comment" class="w-6 h-6">
            ...
            @foreach ($topUsers as $index => $user)
                <div class="flex items-center gap-4">
                    ...
                    @else
                        ...
                        @endphp
                        <div class="w-10 h-10 flex items-center justify-center rounded-full border-2 border-dark shadow-border-offset text-white font-bold {{ $bg }}">
                            {{ $initial }}
                        </div>
                    @endif
                    <div>
        ...
        <div class="bg-white p-6 rounded-playful-lg border-2 border-dark max-w-xl w-full shadow-border-offset overflow-y-auto max-h-[90vh]">
            ...
            <div class="mt-6 pt-4 border-t border-gray-200">
                <form id="addCommentForm" class="flex gap-2">
                    ...
                    <div class="flex-1">
                        <input type="text" name="content" placeholder="Tulis komentar..." class="w-full p-3 rounded-playful-lg border-2 border-dark shadow-border-offset font-lexend focus:outline-none">
                    </div>
                    <button type="submit" class="bg-primary text-white px-4 py-3 rounded-playful-lg border-2 border-dark font-lexend text-sm shadow-border-offset hover:bg-primary/80 transition-all active:shadow-[1px_2px_0px_#1A1A40] active:translate-x-0.5 active:translate-y-0.5">
                        Kirim
                    </button>
                </form>
            </div>
    ...
    <!-- Create Post Modal -->
    <!-- Modal Create Post -->
    <div id="createPostModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 hidden">
        <div class="bg-white p-6 rounded-playful-lg border-2 border-dark max-w-xl w-full shadow-border-offset overflow-y-auto max-h-[90vh]">
            ...
            <!-- Form Create Post -->
            <form action="{{ route('user.comunity.store') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                @csrf

                <!-- Judul -->
                <div>
                    <label class="block font-lexend text-dark font-semibold mb-2">Judul</label>
                    <input type="text" name="title" class="w-full p-3 rounded-playful-lg border-2 border-dark shadow-border-offset font-lexend focus:outline-none" placeholder="Masukkan judul postingan...">
                </div>

                <!-- Konten -->
                <div>
                    <label class="block font-lexend text-dark font-semibold mb-2">Konten</label>
                    <textarea name="content" rows="5" class="w-full p-3 rounded-playful-lg border-2 border-dark shadow-border-offset font-lexend focus:outline-none resize-none" placeholder="Tulis sesuatu..."></textarea>
                </div>

                <!-- Upload Gambar -->
                <div>
                    <label class="block font-lexend text-dark font-semibold mb-2">Upload Gambar (Opsional)</label>
                    <input type="file" name="image" class="w-full border-2 border-dark rounded-playful-lg p-2 font-lexend shadow-border-offset bg-gray-50 focus:outline-none">
                </div>

                <!-- Checkbox Anonymous -->
                <div class="flex items-center gap-2">
                    <input type="checkbox" name="is_anonymous" value="1" id="is_anonymous" class="w-4 h-4 border-2 border-dark rounded">
                    <label for="is_anonymous" class="font-lexend text-dark text-sm">Posting sebagai anonim</label>
                </div>

                <!-- Tombol Simpan -->
                <div class="flex justify-end gap-3">
                    <button type="button" onclick="closeCreatePostModal()" class="px-4 py-2 rounded-playful-lg border-2 border-dark shadow-border-offset font-lexend text-sm bg-gray-200 hover:bg-gray-300 transition-all">
                        Batal
                    </button>
                    <button type="submit" class="px-4 py-2 rounded-playful-lg border-2 border-dark shadow-border-offset font-lexend text-sm bg-primary text-white hover:bg-primary/80 transition-all active:shadow-[1px_2px_0px_#1A1A40] active:translate-x-0.5 active:translate-y-0.5">
                        Simpan
                    </button>
                </div>
            </form>
        ...
        function toggleLike(postId) {
            fetch(`/post/${postId}/like`, {
                ...
                .then(data => {
                    ...
                    if (data.liked) {
                        likeIcon.innerHTML = `<path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41 0.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.18L12 21.35z" />`;
                        likeIcon.classList.add('text-red-500');
                        likeIcon.classList.remove('text-gray-500');
                        btn.setAttribute('data-liked', 'true');
                    } else {
                        likeIcon.innerHTML = `<path stroke-linecap="round" stroke-linejoin="round" d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733C11.285 4.876 9.623 3.75 7.688 3.75 5.099 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z" />`;
