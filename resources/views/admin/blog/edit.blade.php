...
<div class="container mx-auto px-6 py-8">
    <div class="flex justify-between items-center mb-8">
        <div>
            <h1 class="text-h2 font-bold text-dark">Edit Blog</h1>
            <p class="text-gray-600 mt-1">Edit artikel blog yang sudah ada.</p>
        </div>
                ...
                <div class="space-y-6">
                        ...
                        <div class="border-2 border-dashed border-dark rounded-playful-sm p-4 text-center">
                            @if($blog->featured_image)
                                <div id="image-preview" class="mb-3">
                                    <img id="preview" src="{{ asset('storage/' . $blog->featured_image) }}" class="max-w-full h-48 object-cover rounded-playful-sm border-2 border-dark mx-auto">
                                </div>
                            @else
                            ...
                            <button type="button" onclick="document.getElementById('featured_image').click()" 
                                ...
                                {{ $blog->featured_image ? 'Ganti Gambar' : 'Pilih Gambar' }}
                            </button>
                    ...
                    <div>
                        ...
                        <textarea id="excerpt" name="excerpt" rows="4" 
                                  class="w-full border-2 border-dark rounded-playful-sm p-3 focus:outline-none focus:ring-2 focus:ring-primary"
                                  placeholder="Ringkasan singkat tentang artikel...">{{ old('excerpt', $blog->excerpt) }}</textarea>
                        @error('excerpt')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
